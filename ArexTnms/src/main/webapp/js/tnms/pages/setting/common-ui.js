(function(){
  ...
  async function saveSeveritySettings(){
    ...
    for(let index=0;index<rows.length;index++){
      ...
      await api('/setting/common-ui/code.ajax',{method:'POST',body:new URLSearchParams({comCdGroupId:'SYS_STTS',comCd:row.dataset.severityCode,comCdNm:row.querySelector('.severity-name').value.trim(),comCdExpln:x.comCdExpln||'',comCdSeq:String(index+1),comCdUseYn:'Y',ext1Cn:hex,ext2Cn:x.ext2Cn||''})});
    }
  }
  window.saveUiSection=async function(){try{await saveUiSettings();showToast('운영 · UI 설정을 저장했습니다.');await window.loadSettings(data.selectedGroupId)}catch(e){showToast(e.message,true)}};
