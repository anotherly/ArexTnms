...
public class UserAccountApplicationServiceImpl implements UserAccountApplicationService {
    private static final String DEFAULT_AUTHRT_CD = "VIEWER";
    ...
    @Resource(name = "userAccountApplicationMapper") private UserAccountApplicationMapper applicationMapper;
    ...
    public String submit(UserAccountApplicationVO application) {
        ...
        if (applicationMapper.countActiveAuthrt(DEFAULT_AUTHRT_CD) != 1) {
            throw new IllegalStateException("기본 조회 사용자 권한이 없습니다. 권한 설정을 확인해 주세요.");
        }
        ...
        application.setDmndAuthrtCd(DEFAULT_AUTHRT_CD);
        application.setAplyRsn("로그인 화면 계정 신청");
        application.setAplySttsCd(STATUS_WAITING);
        if ("INTERNAL".equals(application.getUserSeCd())) {
        ...
        if (applicationMapper.insertAccountApplication(application) != 1) throw new IllegalStateException("계정 신청 저장에 실패했습니다.");
    ...
    public List<UserAccountApplicationVO> selectApplicationList(UserAccountApplicationVO searchVO) {
        ...
        searchVO.setAplySttsCd(upper(trim(searchVO.getAplySttsCd())));
    ...
    @Override
    public UserAccountApplicationVO selectApplication(String aplyNo) {
        String normalizedAplyNo = trim(aplyNo);
        if (normalizedAplyNo == null) throw new IllegalArgumentException("계정 신청 정보가 없습니다.");
        UserAccountApplicationVO application = applicationMapper.selectApplication(normalizedAplyNo);
    ...
    public void approve(String aplyNo, UserVO actor) {
        ...
        if (userMapper.countByUserId(application.getUserId()) > 0) throw new IllegalArgumentException("동일한 사용자 아이디의 계정이 이미 존재합니다.");
        if (applicationMapper.countActiveAuthrt(application.getDmndAuthrtCd()) != 1) {
            throw new IllegalStateException("신청된 권한 정보를 찾을 수 없습니다: " + application.getDmndAuthrtCd());
        }
        ...
        user.setUserSeCd(application.getUserSeCd());
        user.setOgdpBzentyNm(application.getOgdpBzentyNm());
        user.setDeptNm(application.getDeptNm());
        user.setMobileNo(application.getMobileNo());
        user.setTelno(application.getTelno());
        user.setEmlAddr(application.getEmlAddr());
        user.setUserSttsCd("NORMAL");
        user.setUseYn("Y");
        user.setAuthrtCd(application.getDmndAuthrtCd());
        user.setRgtrId(actor.getUserId());

        if (userMapper.insertUser(user) != 1) throw new IllegalStateException("승인 계정 생성에 실패했습니다.");
        application.setAprvUserId(actor.getUserId());
        if (applicationMapper.approveApplication(application) != 1) throw new IllegalStateException("계정 신청 승인 상태 변경에 실패했습니다.");
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public void reject(String aplyNo, String reason, UserVO actor) {
        requireActor(actor);
        UserAccountApplicationVO application = getPendingApplicationForUpdate(aplyNo);
        ...
        if (applicationMapper.rejectApplication(application) != 1) throw new IllegalStateException("계정 신청 반려 상태 변경에 실패했습니다.");
    }

    private UserAccountApplicationVO getPendingApplicationForUpdate(String aplyNo) {
        String normalizedAplyNo = trim(aplyNo);
        if (normalizedAplyNo == null) throw new IllegalArgumentException("처리할 계정 신청을 선택해 주세요.");
        UserAccountApplicationVO application = applicationMapper.selectApplicationForUpdate(normalizedAplyNo);
        if (application == null) throw new IllegalArgumentException("존재하지 않는 계정 신청입니다.");
        if (!STATUS_WAITING.equals(application.getAplySttsCd())) throw new IllegalArgumentException("이미 처리된 계정 신청입니다.");
        return application;
    }
    ...
    private void normalize(UserAccountApplicationVO application) {
        ...
        application.setUserSeCd(upper(trim(application.getUserSeCd())));
    ...
    private void validate(UserAccountApplicationVO application) {
        if (application.getUserId() == null || !USER_ID.matcher(application.getUserId()).matches()) throw new IllegalArgumentException("아이디는 영문과 숫자를 조합해 4~20자로 입력해 주세요.");
        if (application.getPassword() == null || !PASSWORD.matcher(application.getPassword()).matches()) throw new IllegalArgumentException("비밀번호는 영문·숫자·특수문자를 포함해 8~30자로 입력해 주세요.");
        if (application.getPassword().toLowerCase().contains(application.getUserId().toLowerCase())) throw new IllegalArgumentException("비밀번호에 사용자 아이디를 포함할 수 없습니다.");
        if (!application.getPassword().equals(application.getPasswordConfirm())) throw new IllegalArgumentException("비밀번호 확인이 일치하지 않습니다.");
        if (application.getUserNm() == null || application.getUserNm().length() > 100) throw new IllegalArgumentException("이름을 입력해 주세요.");
        if (!"INTERNAL".equals(application.getUserSeCd()) && !"EXTERNAL".equals(application.getUserSeCd())) throw new IllegalArgumentException("소속 구분을 선택해 주세요.");
    ...
    private String digits(String value) { String normalized = trim(value); return normalized == null ? null : normalized.replaceAll("[^0-9]", ""); }
    private void requireActor(UserVO actor) {
        if (actor == null || actor.getUserId() == null || actor.getUserId().trim().length() == 0) throw new IllegalStateException("로그인 사용자 정보를 확인할 수 없습니다.");
    }
}
